...
export default function AboutPage() {
  return <main className="overflow-hidden bg-[#fbfcf9] text-brand-ink">
    ...
    <section className="bg-brand-navy text-white"><div className="mx-auto grid max-w-7xl items-center gap-14 px-5 py-24 sm:px-8 md:py-32 lg:grid-cols-[.8fr_1.2fr]"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-green-300">Areas of operation</p><h2 className="mt-4 text-5xl font-semibold leading-[.95] tracking-[-0.05em] md:text-7xl">Closer than<br/>you think.</h2><p className="mt-7 max-w-md text-lg leading-8 text-slate-300">We’re growing Nasse one area at a time. Explore where our cleaning services are currently available.</p><Link href="/areas" className="group mt-8 inline-flex items-center gap-4 rounded-full bg-brand-green px-6 py-3.5 font-bold">Explore our coverage <Arrow/></Link></div><Link href="/areas" aria-label="Explore Nasse service coverage across Uganda" className="group relative mx-auto grid aspect-[4/5] w-full max-w-md place-items-center"><div className="absolute inset-8 rounded-[48%_52%_45%_55%/35%_42%_58%_65%] border border-green-300/25 bg-gradient-to-br from-brand-green/80 to-green-800 shadow-[0_0_80px_rgba(114,182,43,.22)] transition-transform duration-500 group-hover:scale-[1.03]"/><div className="relative text-center"><span className="block text-xs font-bold uppercase tracking-[.3em] text-green-100">Uganda</span><span className="mt-2 block text-7xl font-semibold tracking-[-.08em]">UG</span><span className="mt-5 inline-flex rounded-full bg-white/15 px-4 py-2 text-sm backdrop-blur">View live coverage ↗</span></div></Link></div></section>
