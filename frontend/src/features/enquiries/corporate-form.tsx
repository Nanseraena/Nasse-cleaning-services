...
export function CorporateForm(){
 ...
 return <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">{([['company_name','Company name'],['contact_name','Contact person'],['email','Business email'],['phone','Phone'],['facility_type','Facility type'],['location','Location'],['approximate_size','Approximate size'],['frequency','Cleaning frequency']] as const).map(([k,p])=><input key={k} className={cls} placeholder={p} required={['company_name','contact_name','email','phone'].includes(k)} value={f[k]} onChange={e=>set(k,e.target.value)}/>)}<FormattedDateInput label="Preferred start date" value={f.preferred_start_date} onChange={value=>set('preferred_start_date',value)}/><textarea className={`${cls} md:col-span-2`} rows={5} placeholder="Requirements" value={f.requirements} onChange={e=>set('requirements',e.target.value)}/><button className="rounded-xl bg-brand-navy px-5 py-3 font-semibold text-white md:col-span-2">Send Corporate Enquiry</button></form>;
}
